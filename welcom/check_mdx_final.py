...
def check_mdx_fixed(file_path):
    ...
    # Use word boundary \b to avoid matching AccordionGroup as Accordion
    components = ['Accordion', 'AccordionGroup', 'Info', 'Note', 'CodeGroup', 'Table', 'Step', 'Steps', 'Card', 'CardGroup']
    for comp in components:
        open_tags = len(re.findall(rf'<{comp}\b', content))
    ...
    # 3. Check for unescaped curly braces outside code blocks
    ...
    if unescaped_braces:
        ...
        print("Curly braces: Safe (All escaped or in backticks)")

if __name__ == "__main__":
    if len(sys.argv) > 1:
        check_mdx_fixed(sys.argv[1])
    else:
        check_mdx_fixed('/Users/a1234/Desktop/docs/welcom/agent_v2.1.mdx')
